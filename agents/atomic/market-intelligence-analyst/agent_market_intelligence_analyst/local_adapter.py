"""Local adapter — stdin/stdout JSON-lines handler for Platform Router mode.

Protocol:
- Read JSON messages from stdin (one per line).
- Dispatch to the appropriate agent method.
- Write JSON responses to stdout (one per line).

Message format (inbound):
    {"method": "analyze_market", "params": {"data": "...", "framework": "porter"}}
    {"method": "identify_trends", "params": {"data": "..."}}
    {"method": "generate_briefing", "params": {"analysis": {...}}}

Message format (outbound):
    {"status": "ok", "result": {...}}
    {"status": "error", "error": "...", "error_type": "..."}
"""

from __future__ import annotations

import json
import sys
import traceback

from agent_market_intelligence_analyst.agent import MarketIntelligenceAgent
from agent_market_intelligence_analyst.models import MarketAnalysis


def handle_message(agent: MarketIntelligenceAgent, message: dict) -> dict:
    """Dispatch a single inbound message to the agent.

    Args:
        agent: The MarketIntelligenceAgent instance.
        message: Parsed JSON message from stdin.

    Returns:
        Response dict to be serialized as JSON-lines to stdout.
    """
    method = message.get("method", "")
    params = message.get("params", {})

    try:
        if method == "analyze_market":
            data = params.get("data", "")
            framework = params.get("framework", "porter")
            if not data:
                return {"status": "error", "error": "Missing 'data' parameter"}
            result = agent.analyze_market(data, framework)
            return {"status": "ok", "result": result.model_dump()}

        elif method == "identify_trends":
            data = params.get("data", "")
            if not data:
                return {"status": "error", "error": "Missing 'data' parameter"}
            result = agent.identify_trends(data)
            return {"status": "ok", "result": result.model_dump()}

        elif method == "generate_briefing":
            raw_analysis = params.get("analysis", {})
            if not raw_analysis:
                return {"status": "error", "error": "Missing 'analysis' parameter"}
            analysis = MarketAnalysis.model_validate(raw_analysis)
            result = agent.generate_briefing(analysis)
            return {"status": "ok", "result": result.model_dump()}

        else:
            return {"status": "error", "error": f"Unknown method: {method}"}
    except Exception as exc:
        return {
            "status": "error",
            "error": str(exc),
            "error_type": type(exc).__name__,
        }


def run_local_adapter() -> None:
    """Run the local adapter, reading JSON-lines from stdin.

    Reads lines until EOF, dispatches each to the agent, and writes
    JSON-line responses to stdout.
    """
    agent = MarketIntelligenceAgent()

    for line in sys.stdin:
        line = line.strip()
        if not line:
            continue

        try:
            message = json.loads(line)
        except json.JSONDecodeError as e:
            response = {"status": "error", "error": f"Invalid JSON: {e}"}
        else:
            try:
                response = handle_message(agent, message)
            except Exception as exc:
                traceback.print_exc(file=sys.stderr)
                response = {
                    "status": "error",
                    "error": str(exc),
                    "error_type": type(exc).__name__,
                }

        sys.stdout.write(json.dumps(response, ensure_ascii=False) + "\n")
        sys.stdout.flush()
