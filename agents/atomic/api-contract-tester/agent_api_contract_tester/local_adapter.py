...
Message format (inbound):
    {"method": "validate_contract", "params": {"spec_content": "..."}}
...
def handle_message(agent: ApiContractTesterAgent, message: dict) -> dict:
    """Dispatch a single inbound message to the agent.

    Args:
        agent: The ApiContractTesterAgent instance.
    ...
    try:
        if method == "validate_contract":
            ...
            if not spec_content:
                return {"status": "error", "error": "Missing 'spec_content' parameter"}
