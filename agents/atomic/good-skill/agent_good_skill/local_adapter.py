...
Message format (inbound):
    {"method": "run", "params": {"task": "...", "context": {...}}}
...
def handle_message(agent: GoodSkillAgent, message: dict) -> dict:
    """Dispatch a single inbound message to the agent.

    Args:
        agent: The GoodSkillAgent instance.
    ...
    try:
        if method == "run":
            ...
            if not task:
                return {"status": "error", "error": "Missing 'task' parameter"}
            context = params.get("context")
            result = asyncio.run(agent.run(task, context))
            return {"status": "ok", "result": {"output": result}}
