...
## Target Project Structure (when code begins)

```
platform/          # Core platform
├── router/        # Platform Router (4-Phase Workflow)
├── orchestration/ # Self-built orchestration: TaskGraph, ProcessManager, IPC, OrchestrationDSL, SubtaskController
├── gateway/       # MCP Gateway aggregation
├── config/        # Model configuration (Provider Registry + pydantic-ai)
├── local/         # Local Platform (CLI + Git Installer + Supervisor)
│   ├── cli.py     # CLI entry (Typer)
│   ├── sources.py # Package sources (sources.yaml)
│   ├── installer.py # Git Installer (clone --sparse + venv)
│   ├── lockfile.py # Lock file management (lockfile.json)
│   ├── supervisor.py # Agent process management
│   └── config.py  # Configuration (config.toml + Provider Registry)
├── skills/        # Skill loader
├── evolution/     # Self-Evolution Engine
├── runtime/       # Python Runtime (CaveAgent integration)
└── models/        # Shared data models
agents/            # Official Agents (10 atomic + 5 composite)
