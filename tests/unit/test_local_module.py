...
Covers CLI commands and cross-module regression tests.
Per-module tests live in dedicated files:
  - test_local_lockfile.py (LockfileManager)
  - test_local_sources.py (SourceManager)
  - test_local_installer.py (GitInstaller)
  - test_local_supervisor.py (AgentSupervisor, RestartTracker)
"""
...
def _make_mock_pm() -> MagicMock:
    ...
    pm = MagicMock(
        spec=["start_agent", "stop_agent", "stop_all", "get_agent", "list_running", "health_check"]
    )
...
class TestCLI:
    ...
    def test_sources_list(self) -> None:
        ...
        mock_sources.list_sources.return_value = [
            SourceEntry(
                name="official", type="git", url="https://example.com/repo.git", branch="main"
            ),
        ]
        mock_loader = MagicMock()
    ...
    def test_sources_add_success(self) -> None:
        ...
        with (
            ...
            result = runner.invoke(
                app, ["sources", "add", "--name", "my-src", "--url", "https://x.com/r.git"]
            )
...
class TestRunGitUsesCommunicate:
    ...
    async def test_run_git_includes_stderr_on_failure(self) -> None:
        mock_proc = AsyncMock()
        mock_proc.communicate = AsyncMock(return_value=(b"", b"error: pathspec 'x' did not match"))
    ...
    async def test_run_git_capture_includes_stderr_on_failure(self) -> None:
        mock_proc = AsyncMock()
        mock_proc.communicate = AsyncMock(return_value=(b"", b"remote: Repository not found"))
        mock_proc.returncode = 128
...
class TestLockfileManagerGetEntryFrom:
    ...
    def test_get_entry_from_missing_returns_none(self, tmp_path: Path) -> None:
...
class TestGitInstallerInstall:
    ...
    @pytest.mark.parametrize(
        "name",
        ["bad agent name", "../etc/passwd", ".hidden-agent", "foo/bar"],
        ids=["spaces", "path-traversal", "leading-dot", "slash"],
    )
    async def test_install_invalid_name(self, mock_installer, name: str) -> None:
        """install() raises InstallationError for invalid agent names."""
    ...
    async def test_install_rollback_on_failure(self, mock_installer, tmp_path: Path) -> None:
        ...
        installer._validate_agent_package = MagicMock(
            return_value=(
                ["No manifest found. Expected one of: agent.toml, agent-manifest.yaml"],
                {},
            )
        )
...
class TestRmTreeBestEffort:
    ...
    async def test_uninstall_continues_on_rmtree_error(self, tmp_path: Path) -> None:
